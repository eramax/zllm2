...
fn fmt(out: *std.ArrayList(u8), allocator: std.mem.Allocator, comptime f: []const u8, args: anytype) !void {
...
pub fn render(allocator: std.mem.Allocator, model: *const c.llama_model) ![]u8 {
    ...
    try fmt(&out, allocator, "┌─ {s} ──────────────────────────────────────────┐\n", .{desc});
    try fmt(&out, allocator, "│ arch: {s} | params: {d:.1}B | size: {d:.1} GB", .{ arch, params_b, size_gb });
    try padTo(&out, allocator, 56);
    ...
    try fmt(&out, allocator, "│ embd: {d} | layers: {d} | heads: {d} | kv: {d} | dim: {d}", .{ n_embd, n_layer, n_head, n_head_kv, head_dim });
    try padTo(&out, allocator, 56);
    try out.appendSlice(allocator, "│\n");
...
fn renderLlamaFamily(
    ...
    try out.appendSlice(allocator, "│ ");
    if (rope_base.len > 0) {
        try fmt(out, allocator, "rope: base={s} scale={d:.1}", .{ rope_base, rope_scale });
    }
    try fmt(out, allocator, " | norm: rms(ε={s}) | act: silu", .{if (eps.len > 0) eps else "?"});
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");

    if (ffn_dim.len > 0) {
        try fmt(out, allocator, "│ ffn: {s} (SwiGLU: gate + up → silu → down)", .{ffn_dim});
    } else {
    ...
    try fmt(out, allocator, "│ [tok_embd]  {d} × {d}", .{ n_embd, n_embd });
    if (vocab_model.len > 0) try fmt(out, allocator, "  vocab: {s}", .{vocab_model});
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");
    ...
    if (n_layer > 1) {
        try fmt(out, allocator, "│ {d:>2}..{d:>2} × ", .{ 0, n_layer - 1 });
    } else {
        try out.appendSlice(allocator, "│  0   × ");
    }
    try out.appendSlice(allocator, "[attn_norm → Q/K/V → RoPE");
    if (gqa) try fmt(out, allocator, " (GQA {d}H/{d}KV)", .{ n_head, n_head_kv });
    ...
    try fmt(out, allocator, "│ [output_norm]  {d}", .{n_embd});
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");

    if (n_embd_out > 0 and n_embd_out != n_embd) {
        try fmt(out, allocator, "│ [output]  {d} × {d}", .{ n_embd, n_embd_out });
    } else {
        try fmt(out, allocator, "│ [output]  {d} × vocab (tied: tok_embd)", .{n_embd});
    }
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");
...
fn renderGemma4(
    ...
    try fmt(out, allocator, "│ rope: base={s} scale={d:.1} | norm: rms(ε={s})", .{ rope_base, rope_scale, if (eps.len > 0) eps else "?" });
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");

    if (ffn_dim.len > 0) {
        try fmt(out, allocator, "│ ffn: {s} (GeGLU: gate → gelu · up → down)", .{ffn_dim});
    }
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");

    try out.appendSlice(allocator, "├────────────────────────────────────────────────────────┤\n");
    try fmt(out, allocator, "│ [tok_embd]  {d} × {d}  (scaled by sqrt({d}))", .{ n_embd, n_embd, n_embd });
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");
    try out.appendSlice(allocator, "│     │                                                  │\n");

    const gqa = n_head_kv > 0 and n_head_kv < n_head;
    try fmt(out, allocator, "│ {d:>2} layers × ", .{n_layer});
    try out.appendSlice(allocator, "[attn_norm → Q/K/V → RoPE");
    if (gqa) try fmt(out, allocator, " GQA({d}/{d})", .{ n_head, n_head_kv });
    ...
    try out.appendSlice(allocator, "│\n");
    try out.appendSlice(allocator, "│     │                                                  │\n");
    try fmt(out, allocator, "│ [output_norm]  {d}  →  [output]  {d} × vocab", .{ n_embd, n_embd });
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");
    try out.appendSlice(allocator, "└────────────────────────────────────────────────────────┘\n");
}
...
fn renderQwen35(
    ...
    try fmt(out, allocator, "│ rope: base={s} scale={d:.1} | norm: rms(ε={s}) | act: silu", .{ rope_base, rope_scale, if (eps.len > 0) eps else "?" });
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");

    if (ffn_dim.len > 0) {
        try fmt(out, allocator, "│ ffn: {s} (SwiGLU)", .{ffn_dim});
    }
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");

    try out.appendSlice(allocator, "├────────────────────────────────────────────────────────┤\n");
    try fmt(out, allocator, "│ [tok_embd]  {d}", .{n_embd});
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");
    try out.appendSlice(allocator, "│     │                                                  │\n");
    ...
    if (moe_layers.len > 0) {
        try fmt(out, allocator, "│   dense: layers [{s}] → SwiGLU FFN", .{moe_layers});
        try padTo(out, allocator, 56);
        try out.appendSlice(allocator, "│\n");
    ...
    try out.appendSlice(allocator, "│     │                                                  │\n");
    try fmt(out, allocator, "│ [output_norm]  {d}  →  [output]", .{n_embd});
    try padTo(out, allocator, 56);
    try out.appendSlice(allocator, "│\n");
    try out.appendSlice(allocator, "└────────────────────────────────────────────────────────┘\n");
}
...
fn padTo(out: *std.ArrayList(u8), allocator: std.mem.Allocator, target: usize) !void {
    ...
    if (visible < target) {
        ...
        while (remaining > 0) {
            ...
            try out.appendSlice(allocator, buf[0..chunk]);
            remaining -= chunk;
        }
    }
}
